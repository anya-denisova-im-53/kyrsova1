...
const signUpBtn = document.querySelector('.join-army-card button, #signUpBtn') || document.querySelector('button[type="submit"]') || Array.from(document.querySelectorAll('button')).find(el => el.textContent.includes('Sign Up'));
...
if (signUpBtn) {
    signUpBtn.onclick = (event) => {
        ...
        if (!email || !password) {
            alert('Будь ласка, заповни всі поля для реєстрації! 💜');
            return;
        }
        ...
        localStorage.setItem('army_user_email', email);

    
        console.log(`%c [ARMY Registry] New user registered: ${email}`, 'color: #9b59b6; font-weight: bold;');
        
        alert(`Ласкаво просимо до ARMY, ${email}! 💜 Твій профіль успішно створено.`);
